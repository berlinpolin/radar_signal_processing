...
def main():
    ...
    radar_raw_data_cube_range_velocity_fft = \
        np.fft.fftshift(radar_raw_data_cube_range_velocity_fft)
    ...
    freq_x = np.fft.fftshift(np.fft.fftfreq(int(num_samp_per_chirp)))  # Frequency values for y-axis
    freq_y = np.fft.fftshift(np.fft.fftfreq(int(num_chirp_per_frame)))  # Frequency values for x-axis
